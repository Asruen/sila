...
async function fetchAllMenus() {
    try {
        ...
        if (Array.isArray(menuData) && menuData.length > 0) {
            ...
            startTabSwitching(); // Mulai pergantian tab setelah data siap
            rotateMenu();
        } else {
...
//Auto Refresh setelah semua data ditampilkan
function rotateMenu() {
    setTimeout(() => {
        currentMenuIndex++;
    ...
    if (tableBody) {
        ...
        data.forEach((item, index) => {
            ...
            row.innerHTML = `<td>${index + 1}</td>
                             ...
                             <td>${item.tanggal_jam ? item.tanggal_jam + date(d/m/y H:i) : "-"}</td>
